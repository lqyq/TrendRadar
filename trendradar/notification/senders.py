...
def send_to_email(
    ...
    *,
    ...
    retry_interval: float = 2.0  # 新增：基础重试间隔（秒）
) -> bool:
    """
    发送邮件通知
    ...
    Args:
        ...
        retry_interval: 基础重试间隔（指数退避，默认2秒）
    ...
    now = get_time_func() if get_time_func else smtplib.datetime.datetime.now()
...
请使用支持HTML的邮件客户端查看完整报告内容。
    ...
    while retry_count < max_retries:
        ...
        try:
            # 建立 SMTP 连接
            if use_tls:
                server = smtplib.SMTP(smtp_server, smtp_port, timeout=60)  # 超时延长到60秒
                server.set_debuglevel(0)
                server.ehlo()
                # 启用 TCP 保活，防止连接被断开
                server.sock.setsockopt(socket.SOL_SOCKET, socket.SO_KEEPALIVE, 1)
                if hasattr(socket, 'TCP_KEEPIDLE'):
                    server.sock.setsockopt(socket.IPPROTO_TCP, socket.TCP_KEEPIDLE, 10)
                    server.sock.setsockopt(socket.IPPROTO_TCP, socket.TCP_KEEPINTVL, 5)
                    server.sock.setsockopt(socket.IPPROTO_TCP, socket.TCP_KEEPCNT, 3)
                server.starttls()
                server.ehlo()
            else:
            ...
            # 替换 send_message 为 sendmail，更稳定
            ...
            if retry_count < max_retries:
                wait_time = retry_interval * (2 ** (retry_count - 1))  # 指数退避：2s → 4s → 8s
                print(f"⚠️ 服务器断开连接，第 {retry_count} 次重试（等待 {wait_time} 秒）...")
                time.sleep(wait_time)
            else:
                print("❌ 邮件发送失败：服务器意外断开连接（重试次数耗尽）")
                return False
            ...
            return False  # 收件人错误无需重试

        except smtplib.SMTPSenderRefused as e:
            print(f"❌ 邮件发送失败：发件人地址被拒绝 {e}")
            return False  # 发件人错误无需重试

        except smtplib.SMTPDataError as e:
            print(f"❌ 邮件发送失败：邮件数据错误 {e}")
            return False  # 数据错误无需重试

        except smtplib.SMTPConnectError as e:
            retry_count += 1
            if retry_count < max_retries:
                wait_time = retry_interval * (2 ** (retry_count - 1))
                print(f"⚠️ 无法连接SMTP服务器，第 {retry_count} 次重试（等待 {wait_time} 秒）...")
                print(f"详细错误: {str(e)}")
                time.sleep(wait_time)
            else:
            ...
            if retry_count < max_retries:
                wait_time = retry_interval * (2 ** (retry_count - 1))
                print(f"⚠️ 发送异常，第 {retry_count} 次重试（等待 {wait_time} 秒）...")
                print(f"异常信息: {str(e)}")
                time.sleep(wait_time)
            else:
    ...
    # 所有重试都失败
    print(f"❌ 邮件发送失败 [{report_type}]：已重试 {max_retries} 次仍未成功")
    return False
